...
def create_output_html():
    ...
    f.write('<style> body { background-image: url("emerald.png"); } table { background-color: seagreen; color: black; margin: 0 auto; } </style>\n')  

    f.write('<h1 style="text-align: center;">Guest Data Report - Emerald Beach Hotel & Resort</h1>\n')
    ...
    for g in guest:
        f.write(tr + td.join([str(x) for x in g]) + endtr)
